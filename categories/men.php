...
foreach ($categories as $category) {
    $stmt = $con->prepare("
        ...
        WHERE c.name = ? AND p.gender = 'male'
    ");
    $stmt->bind_param("s", $category);
    $stmt->execute();
    $result = $stmt->get_result();
    
    $allProducts[$category] = $result->fetch_all(MYSQLI_ASSOC);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<header class="header">
    ...
    <nav class="main-nav active" id="mainNav">
        <ul class="nav-links">
            ...
            <li class="dropdown">
                <a href="#" class="nav-link has-dropdown"><span>Men</span>
                    <i class="fas fa-chevron-down dropdown-arrow hidden"></i>
                    <i class="fa-solid fa-arrow-right"></i>
                </a>
                <ul class="dropdown-menu">
...
<main class="product-listing">
    ...
    <?php foreach ($allProducts as $categoryName => $products): ?>
